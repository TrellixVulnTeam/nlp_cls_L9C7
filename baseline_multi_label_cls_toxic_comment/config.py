...
def parse_args():
    ...
    bert_type = "base"
    bert_path = "/media/data1/luokunhao/pretrained_model/bert-base-uncased"
    learning_rate = 1e-5 if bert_type == "large" else 3e-5
    test_steps    = 500  if bert_type == "large" else 200
    adam_epsilon  = 1e-8
    per_train_bsz = 1    if bert_type == "large" else 8
    per_dev_bsz   = 8

    # # Required parameters
    checkpoint = os.path.join(par_dir, f"checkpoint/baseline/toxic_comment/bert_{bert_type}/01")
