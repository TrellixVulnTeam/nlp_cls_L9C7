...
class BertForMultipleChoice_MLP(PreTrainedBertModel):
    """BERT model for multiple choice tasks.
    This module is composed of the BERT model with a linear layer on top of
    the pooled output.
    Params:
    ...
    def __init__(self, config, num_choices, mlp_hidden_dim = 64, mlp_dropout = 0.25):
        super(BertForMultipleChoice_MLP, self).__init__(config)
        self.num_choices = num_choices
        self.bert = BertModel(config)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        ...
        logits = self.classifier(pooled_output)
        reshaped_logits = logits.view(-1, self.num_choices)
        # reshaped_logits: batch_size * num_labels
...
class BertForQA(PreTrainedBertModel):
    def __init__(self, config):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        pooled_output = self.dropout(pooled_output)
        type_logits = self.classifier(pooled_output)

        if start_positions is not None and \
                end_positions is not None and \
                answer_type is not None:
            ...
            loss_fct = CrossEntropyLoss(ignore_index=ignored_index)
            type_loss = loss_fct(type_logits.view(-1, 4), answer_type.view(-1))

            total_loss = pos_loss + type_loss
            return total_loss
        else:
